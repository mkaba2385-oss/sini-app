...
def test_create_parcelle_success(service: ParcelleService) -> None:
    ...
    assert parcelle.owner_id == 1


def test_get_parcelle_by_id_success(
...
def test_get_parcelle_not_found_raises_exception(
    service: ParcelleService,
) -> None:
...
def test_list_and_filter_parcelles(
    service: ParcelleService,
) -> None:
    p1 = ParcelleCreate(
        name="P1",
...
def test_update_parcelle_success(
    service: ParcelleService,
) -> None:
    payload = ParcelleCreate(
        name="Champ Test",
...
def test_update_parcelle_no_change(
    service: ParcelleService,
) -> None:
    payload = ParcelleCreate(
        name="Inchangé",
...
def test_delete_parcelle_success(
    service: ParcelleService,
) -> None:
    payload = ParcelleCreate(
        name="À Supprimer",
...
def test_update_and_delete_not_found(
    ...
    with pytest.raises(EntityNotFoundError):
        service.updated_parcelle(
            999,
            ParcelleUpdate(
                name="Parcelle Inexistante",
            ),
        )
...
def test_verifier_et_alerter_observer_and_strategy(
    service: ParcelleService,
) -> None:
...
def test_custom_strategy_no_alert(
    service: ParcelleService,
) -> None:
    """Vérifie qu'aucune alerte n'est émise."""

    service.alert_strategy = HighTemperatureAlertStrategy(
        temp_threshold=50.0,
    )
